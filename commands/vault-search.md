...
Procedure:
1. Call `vault_search` with the query. If the query looks like an error message, use the literal error text + stack name. If it's a concept (e.g. "cache strategy"), filter `dirs=["arch"]`.
