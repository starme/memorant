...
1. **Dedup**: call `vault_search` with the decision topic (filter `dirs=["arch"]`). If a similar ADR exists, tell the user — maybe this supersedes it instead of being new.
...
3. If this ADR **supersedes** an old one: set frontmatter `supersedes: "adr-NNN"`, then after creating, call `vault_update_frontmatter(path="arch/<old-adr-file>.md", key="status", value="superseded")` to flip the old one.
4. Call `vault_create_entry(type="arch", ...)`. **Do not pass a sequence** — the server auto-assigns from `arch/.sequence`. Confirm the resulting `adr-NNN` number to the user.
5. Link `related` to落地 snippets and踩坑 bugs if they exist.
