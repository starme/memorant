...
def migration_memory_id(legacy_path: str) -> str:
    ...
    return hashlib.sha256(f"{_MEMORY_ID_PREFIX}{legacy_path}:0".encode()).hexdigest()[
        :32
    ]
...
def detect_legacy_data(root: str | Path) -> dict[str, int]:
    ...
    for directory in LEGACY_DIRS:
        ...
        if not path.is_dir():
            counts[directory] = 0
            continue
        counts[directory] = sum(1 for p in path.rglob("*.md") if p.is_file())
    return counts
...
def _resolve_inside(root: Path, rel: str) -> Path:
    ...
    try:
        inside = os.path.commonpath((str(root_resolved), str(resolved))) == str(
            root_resolved
        )
    except ValueError:
        inside = False
...
def _append_synthetic_event(
    root: Path, legacy_path: str, excerpt: str
) -> dict[str, Any]:
    ...
    for directory in LEGACY_DIRS:
        ...
        for path in sorted(base_dir.rglob("*.md")):
            ...
            if directory == "daily":
                ...
                if not cues:
                    units.append({"legacy_path": rel, "source": "daily", "cues": []})
                else:
...
def _migrate_one(root: Path, unit: dict[str, Any], dry_run: bool) -> dict[str, Any]:
    ...
    if existing is not None:
        ...
        return {
            ...
            "reason": "target exists with different fingerprint",
        }
...
def _write_report(root: Path, timestamp: str, summary: dict[str, Any]) -> str:
    ...
    for item in summary["items"]:
        ...
        reason = _safe_error_message(RuntimeError(str(item.get("reason") or "")), root)
