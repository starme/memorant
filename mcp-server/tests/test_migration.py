...
def _make_legacy(root: Path) -> Path:
    ...
    _write(root / "bugs" / "pytest-flaky.md", _bug("pytest 偶发超时", "## 根因\n\n网络抖动导致。", "resolved"))
    _write(root / "snippets" / "rate-limit.md", _snippet("限流", "## 做法\n\n令牌桶。"))
    _write(root / "arch" / "adr-001-cache.md", _arch("选本地优先", "## Decision\n\n本地文件。", "active"))
    _daily(root / "daily" / "2026-07-24.md", "## 踩坑/线索\n【待升bugs】docker 端口映射失效\n【待升snippets】限流算法\n\n正文流水")
    return root
...
def test_detect_legacy_data_empty_when_missing(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root = tmp_path / "empty"
    monkeypatch.setenv("MEMORANT_ROOT", str(root))
...
def test_arch_deprecated_maps_to_superseded_lifecycle(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root = tmp_path / "memorant"
    monkeypatch.setenv("MEMORANT_ROOT", str(root))
    _write(root / "arch" / "adr-001-old.md", _arch("旧决策", "## Decision\n\n旧的。", "deprecated"))
...
def test_conflict_does_not_overwrite(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root = tmp_path / "memorant"
    monkeypatch.setenv("MEMORANT_ROOT", str(root))
    ...
    _write(root / "memories" / f"{memory_id}.md", "---\ntype: memory\n---\nconflicting\n")
...
def test_failure_isolation_continues_other_entries(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root = tmp_path / "memorant"
    monkeypatch.setenv("MEMORANT_ROOT", str(root))
...
def test_daily_extracts_cue_lines_as_provisional(legacy_root: Path) -> None:
    ...
    daily_mems = [
        m for m in _memories(legacy_root) if (m.get("legacy_path") or "").startswith("daily/")
    ]
    assert daily_mems
    assert all(m["trust_tier"] == "provisional" for m in daily_mems)


def test_daily_pure_noise_skipped(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root = tmp_path / "memorant"
    monkeypatch.setenv("MEMORANT_ROOT", str(root))
    _daily(root / "daily" / "2026-07-24.md", "## 随手记\n\n今天天气不错\n\n没有任何可迁移经验")
    result = migration.migrate_legacy(str(root))
    assert result["migrated"] == 0
...
def test_first_start_hint_none_when_no_legacy(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root = tmp_path / "empty"
    monkeypatch.setenv("MEMORANT_ROOT", str(root))
...
def test_migrate_command_confirm_red_line(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import asyncio
